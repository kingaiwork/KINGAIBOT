...
<header>
  ...
  <div class="connect"><input id="token" type="password" autocomplete="off" placeholder="Admin Token"><button id="connect">连接</button></div>
</header>
<main>
  <section class="hero">
  ...
  <section class="panel">
    <div class="panelHead"><div><div class="eyebrow">GOVERNANCE</div><h2>云策略</h2></div><button id="pull" class="secondary">立即拉取策略</button></div>
...
function render(data){
  const s=(data&&data.state)||{},p=(data&&data.policy)||{};const cards=$('cards');cards.replaceChildren();
  [['设备注册',s.enrolled?'已注册':'未注册'],['Node ID',s.node_id],['Workspace',s.workspace_id],['策略版本',s.policy_version],['最后心跳',time(s.last_heartbeat_at)],['最后同步',time(s.last_sync_at)],['E2EE Sync',s.memory_sync_enabled?'已启用':'未启用'],['Cloud',s.cloud_enabled?'已启用':'本地模式']].forEach(x=>cards.appendChild(card(x[0],x[1])));
