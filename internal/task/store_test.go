...
func TestStoreRejectsTraversalID(t *testing.T) {
	...
	if err := s.Save(&Task{ID: "../escape", Status: Queued}); err == nil {
...
func TestCancelCannotOverwriteTerminalTask(t *testing.T) {
	s, err := NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	if err := s.Save(&Task{ID: "task_1", Status: Completed}); err != nil {
		t.Fatal(err)
	}
	if err := s.Cancel("task_1"); err == nil {
		t.Fatal("expected terminal cancel to fail")
	}
