...
type SyncResponse struct {
	...
	Policy            struct {
		...
		GenericRemoteShell              bool   `json:"generic_remote_shell"`
...
func SettingsFromEnv(dataDir string) (Settings, error) {
	token := strings.TrimSpace(os.Getenv("KINGAI_WORKFORCE_SERVICE_TOKEN"))
	if token == "" {
		return Settings{Enabled: false}, nil
	}
	if !serviceTokenPattern.MatchString(token) {
		return Settings{}, errors.New("KINGAI_WORKFORCE_SERVICE_TOKEN has invalid format")
	}
	base := strings.TrimSpace(os.Getenv("KINGAI_WORKFORCE_URL"))
	if base == "" {
		base = defaultControlPlane
	}
	bindings := strings.TrimSpace(os.Getenv("KINGAI_WORKFORCE_BINDINGS_FILE"))
	if bindings == "" {
		bindings = filepath.Join(dataDir, "workforce", "bindings.json")
	}
	s := Settings{
		Enabled:           true,
...
func NewClient(settings Settings, version string) (*Client, error) {
	if !settings.Enabled {
		return nil, errors.New("workforce client disabled")
	}
	if !serviceTokenPattern.MatchString(settings.ServiceToken) {
		return nil, errors.New("invalid workforce service token")
	}
...
func (c *Client) Heartbeat(ctx context.Context, capabilities []string) error {
	...
	return c.doJSON(ctx, http.MethodPost, "/api/v1/workforce/runtime/heartbeat", map[string]any{
		"version":      c.version,
...
func (c *Client) Sync(ctx context.Context) (*SyncResponse, error) {
	...
	if err := c.doJSON(ctx, http.MethodGet, "/api/v1/workforce/runtime/sync", nil, &out); err != nil {
		return nil, err
	}
	if !out.OK {
		return nil, errors.New("workforce sync not acknowledged")
	}
	if out.Policy.ArbitraryShell || out.Policy.GenericRemoteShell || !out.Policy.CloudNeverBypassesLocalApproval || out.Policy.CredentialsInCloud || out.Policy.ConnectorConfigGrantsPermission {
		return nil, errors.New("unsafe workforce cloud policy rejected")
	}
	if out.Schema != "kingai.workforce.v2" {
		return nil, fmt.Errorf("unsupported workforce schema %q", out.Schema)
	}
	return &out, nil
}
...
func (c *Client) PullTask(ctx context.Context) (*CloudTask, error) {
	...
	if err := c.doJSON(ctx, http.MethodPost, "/api/v1/workforce/runtime/tasks/pull", map[string]any{}, &out); err != nil {
		return nil, err
	}
	return out.Task, nil
}
...
func (c *Client) ReportResult(ctx context.Context, taskID, status string, output any, errorText string) error {
	...
	return c.doJSON(ctx, http.MethodPost, "/api/v1/workforce/runtime/tasks/"+url.PathEscape(taskID)+"/result", body, &out)
}
...
func (c *Client) doJSON(ctx context.Context, method, path string, input, output any) error {
	...
	req.Header.Set("User-Agent", "KINGAIBOT/"+c.version+" enterprise-workforce-v2")
	if input != nil {
...
func validateControlPlaneURL(raw string, allowHTTP bool) error {
	...
	if u.Hostname() == "" || u.User != nil || u.RawQuery != "" || u.Fragment != "" {
		return errors.New("URL requires hostname and must not contain credentials, query or fragment")
	}
	if u.Scheme == "https" {
		return nil
	}
	...
	return errors.New("insecure http allowed only for loopback development")
}
...
func envDuration(name string, def, min, max time.Duration) time.Duration {
	return time.Duration(envInt(name, int(def/time.Second), int(min/time.Second), int(max/time.Second))) * time.Second
}
