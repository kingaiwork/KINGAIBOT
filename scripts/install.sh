...
if command -v cosign >/dev/null 2>&1; then
  ...
  echo "KINGAGENT_REQUIRE_SIGNATURE=1 but cosign is not installed." >&2; exit 3
else
  echo "WARNING: cosign not found; initial install is checksum-verified only. Set KINGAGENT_REQUIRE_SIGNATURE=1 to fail closed." >&2
fi

tar -xzf "$TMP/$ASSET" -C "$TMP"
for required in kingagentd kingagent kingworker kingconsole kingdesktop; do
  [[ -f "$TMP/$required" ]] || { echo "Release asset is missing $required" >&2; exit 4; }
done
...
if [[ "$OS" == linux ]]; then
  ...
  for bin in kingagentd kingagent kingworker kingconsole kingdesktop; do install -m 0755 "$TMP/$bin" "/usr/local/bin/$bin"; done
  install -d -m 0750 -o kingagent -g kingagent /var/lib/kingagent /var/lib/kingagent/workspace
  install -d -m 0750 /etc/kingagent /usr/local/lib/kingagent
  if [[ ! -f /etc/kingagent/config.json ]]; then
    cp "$TMP/config.example.json" /etc/kingagent/config.json
    sed -i -e 's#\./data/workspace#/var/lib/kingagent/workspace#g' -e 's#\./data#/var/lib/kingagent#g' /etc/kingagent/config.json
    chmod 0640 /etc/kingagent/config.json
  fi
...
ENV
    ...
    ensure_env_token /etc/kingagent/kingagent.env KINGAGENT_ADMIN_TOKEN
    ensure_env_token /etc/kingagent/kingagent.env KINGAGENT_MCP_TOKEN
    ensure_env_token /etc/kingagent/kingagent.env KINGAGENT_A2A_TOKEN
...
UNIT
  ...
  if [[ -d /usr/share/applications ]]; then
    cat > /usr/share/applications/kingai-control-center.desktop <<'DESKTOP'
[Desktop Entry]
Type=Application
Name=KING AI Control Center
Comment=Visual client for KINGAIBOT
Exec=/usr/local/bin/kingdesktop
Terminal=false
Categories=Utility;Development;
StartupNotify=true
DESKTOP
    chmod 0644 /usr/share/applications/kingai-control-center.desktop
  fi

  systemctl daemon-reload
  systemctl enable --now kingagent.service
  systemctl enable --now kingagent-update.timer
  echo "Installed KINGAIBOT runtime and visual client."
  echo "Visual client: kingdesktop"
  echo "Control Center: http://127.0.0.1:18889/ui/"
  echo "Configure your model API key in /etc/kingagent/kingagent.env and restart: sudo systemctl restart kingagent"
  echo "Health: curl http://127.0.0.1:18888/healthz"
else
  PREFIX="${HOME}/.local/kingagent"
  PLIST="${HOME}/Library/LaunchAgents/com.kingai.agentos.plist"
  mkdir -p "$PREFIX/bin" "$PREFIX/data/workspace" "$HOME/Library/LaunchAgents" "$HOME/Applications"
  for bin in kingagentd kingagent kingworker kingconsole kingdesktop; do install -m 0755 "$TMP/$bin" "$PREFIX/bin/$bin"; done
  install -m 0755 "$TMP/update.sh" "$PREFIX/update.sh"
...
ENV
    ...
    ensure_env_token "$PREFIX/kingagent.env" KINGAGENT_ADMIN_TOKEN
    ensure_env_token "$PREFIX/kingagent.env" KINGAGENT_MCP_TOKEN
    ensure_env_token "$PREFIX/kingagent.env" KINGAGENT_A2A_TOKEN
...
EOF_UPLIST
  ...
  APP="$HOME/Applications/KING AI Control Center.app"
  mkdir -p "$APP/Contents/MacOS"
  cat > "$APP/Contents/MacOS/kingai-control-center" <<EOF_APP
#!/usr/bin/env bash
exec "$PREFIX/bin/kingdesktop"
EOF_APP
...
EOF_INFO
  ...
  launchctl bootout "gui/$(id -u)/com.kingai.agentos" >/dev/null 2>&1 || true
  launchctl bootout "gui/$(id -u)/com.kingai.agentos.update" >/dev/null 2>&1 || true
  launchctl bootstrap "gui/$(id -u)" "$PLIST"
  launchctl bootstrap "gui/$(id -u)" "$UPDATE_PLIST"
  echo "Installed KINGAIBOT runtime and KING AI Control Center."
  echo "Open: $APP"
  echo "Or run: $PREFIX/bin/kingdesktop"
  echo "Set OPENAI_API_KEY in $PREFIX/kingagent.env, then run: launchctl kickstart -k gui/$(id -u)/com.kingai.agentos"
fi
