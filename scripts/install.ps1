...
try {
  ...
  $root = Join-Path $env:ProgramData "KINGAgent"; $bin = Join-Path $root "bin"; $data = Join-Path $root "data"; $workspace = Join-Path $data "workspace"
  $taskName = "KINGAIBOT"; $updateTask = "KINGAIBOT Update"
  schtasks.exe /End /TN $taskName 2>$null | Out-Null; schtasks.exe /Delete /TN $taskName /F 2>$null | Out-Null
  schtasks.exe /End /TN $updateTask 2>$null | Out-Null; schtasks.exe /Delete /TN $updateTask /F 2>$null | Out-Null
  New-Item -ItemType Directory -Force $bin,$workspace | Out-Null
  Copy-Item (Join-Path $tmp "pkg\kingagentd.exe") (Join-Path $bin "kingagentd.exe") -Force
  Copy-Item (Join-Path $tmp "pkg\kingagent.exe") (Join-Path $bin "kingagent.exe") -Force
  Copy-Item (Join-Path $tmp "pkg\update.ps1") (Join-Path $root "update.ps1") -Force
  $config = Join-Path $root "config.json"
  if (-not (Test-Path $config)) {
    Copy-Item (Join-Path $tmp "pkg\config.example.json") $config
    $j = Get-Content $config -Raw | ConvertFrom-Json
    $j.runtime.data_dir = ($data -replace '\\','/'); $j.runtime.workspace_dir = ($workspace -replace '\\','/')
    $j.security.file_read_roots = @(($workspace -replace '\\','/')); $j.security.file_write_roots = @(($workspace -replace '\\','/'))
    $jsonText = $j | ConvertTo-Json -Depth 20; [IO.File]::WriteAllText($config, $jsonText, (New-Object Text.UTF8Encoding($false)))
  }
  $adminTokenFile = Join-Path $root "admin-token.txt"; $mcpTokenFile = Join-Path $root "mcp-token.txt"; $a2aTokenFile = Join-Path $root "a2a-token.txt"
  Ensure-RandomTokenFile $adminTokenFile; Ensure-RandomTokenFile $mcpTokenFile; Ensure-RandomTokenFile $a2aTokenFile
  ...
  foreach ($tokenFile in @($adminTokenFile,$mcpTokenFile,$a2aTokenFile,$modelKeyFile)) { & icacls.exe $tokenFile /inheritance:r /grant:r "*S-1-5-18:F" "*S-1-5-32-544:F" "*S-1-5-19:R" | Out-Null }
...
"@ | Set-Content -Encoding UTF8 $runScript
  ...
  Register-ScheduledTask -TaskName $taskName -Action $taskAction -Trigger $taskTrigger -Principal $taskPrincipal -Settings $taskSettings -Force | Out-Null; Start-ScheduledTask -TaskName $taskName
  ...
  Write-Host "KINGAIBOT installed to $root"; Write-Host "Runtime identity: NT AUTHORITY\LOCAL SERVICE (not Administrator/SYSTEM)."; Write-Host "Set the model API key in $modelKeyFile (Administrators only), then restart the scheduled task."
