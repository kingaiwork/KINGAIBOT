...
ASSET="king-agent-os_${OS}_${ARCH}.tar.gz"; BASE="https://github.com/${KINGAGENT_REPO}/releases/latest/download"; TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
...
curl -fsSL "$BASE/$ASSET" -o "$TMP/$ASSET"; curl -fsSL "$BASE/SHA256SUMS" -o "$TMP/SHA256SUMS"; verify_checksum
if command -v cosign >/dev/null 2>&1; then curl -fsSL "$BASE/$ASSET.sigstore.json" -o "$TMP/$ASSET.sigstore.json"; cosign verify-blob --bundle "$TMP/$ASSET.sigstore.json" --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity-regexp "$WORKFLOW_IDENTITY_REGEX" "$TMP/$ASSET" >/dev/null
elif [[ "${KINGAGENT_ALLOW_CHECKSUM_ONLY:-0}" != "1" ]]; then echo "cosign is required for unattended updates. Set KINGAGENT_ALLOW_CHECKSUM_ONLY=1 only if you accept checksum-only verification." >&2; exit 3; fi
...
install -m 0755 "$TMP/kingagentd" "$BIN_DIR/kingagentd.new"; install -m 0755 "$TMP/kingagent" "$BIN_DIR/kingagent.new"
[[ -f "$BIN_DIR/kingagentd" ]] && mv "$BIN_DIR/kingagentd" "$BIN_DIR/kingagentd.prev"; [[ -f "$BIN_DIR/kingagent" ]] && mv "$BIN_DIR/kingagent" "$BIN_DIR/kingagent.prev"
mv "$BIN_DIR/kingagentd.new" "$BIN_DIR/kingagentd"; mv "$BIN_DIR/kingagent.new" "$BIN_DIR/kingagent"; "${RESTART_CMD[@]}" || true; sleep 2
POSTCHECK_OK=1; curl -fsS --max-time 5 http://127.0.0.1:18888/healthz >/dev/null || POSTCHECK_OK=0; if [[ $WAS_READY -eq 1 ]]; then curl -fsS --max-time 5 http://127.0.0.1:18888/readyz >/dev/null || POSTCHECK_OK=0; fi
if [[ $POSTCHECK_OK -ne 1 ]]; then [[ -f "$BIN_DIR/kingagentd.prev" ]] && mv "$BIN_DIR/kingagentd.prev" "$BIN_DIR/kingagentd"; [[ -f "$BIN_DIR/kingagent.prev" ]] && mv "$BIN_DIR/kingagent.prev" "$BIN_DIR/kingagent"; "${RESTART_CMD[@]}" || true; echo "Update failed post-update health/readiness continuity check; rolled back." >&2; exit 4; fi
rm -f "$BIN_DIR/kingagentd.prev" "$BIN_DIR/kingagent.prev"; echo "Updated KINGAIBOT: $OLD -> $NEW"
