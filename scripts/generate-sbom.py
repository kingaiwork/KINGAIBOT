...
if source_epoch <= 0:
    try:
        source_epoch = int(subprocess.check_output(["git", "log", "-1", "--format=%ct"], cwd=root, text=True, stderr=subprocess.DEVNULL).strip())
    except Exception:
...
for m in mods:
    if m.get("Main"):
        continue
    path = m.get("Path")
    ver = m.get("Version") or "unknown"
    repl = m.get("Replace")
    if repl:
        path = repl.get("Path", path)
        ver = repl.get("Version") or ver
    if not path:
        continue
...
# GitHub actions/attest identifies CycloneDX SBOMs by bomFormat,
# specVersion and serialNumber. Keep the serial deterministic so release
# artifacts remain reproducible while still satisfying the RFC 4122 form.
serial = "urn:uuid:" + str(uuid.uuid5(uuid.NAMESPACE_URL, f"https://github.com/kingaiwork/KINGAIBOT@{version}:{source_epoch}"))
bom = {
    ...
    "metadata": {
        ...
        "component": {
            ...
            "name": "KINGAIBOT",
            "version": version,
            "bom-ref": f"pkg:generic/king-agent-os@{quote(version, safe='.+~-')}",
        },
    },
